...
let _ = register_translate_type translate_type_without_decay

let translate_binder env (name, typ) =
  { name = name; typ = translate_type env typ; mut = false }

let translate_binders env args =
  List.map (translate_binder env) args

let translate_width = function
  | None -> CInt
...
let translate_branch (env: env) (br: mlbranch) =
  let (pat, guard, expr) = br in
  if guard = None then
...
let translate_branches env branches =
  List.map (translate_branch env) branches

let mk_op_app env w op args =
