...
and (translate_constant : FStar_Extraction_ML_Syntax.mlconstant -> expr) =
  fun c  ->
    ...
    | FStar_Extraction_ML_Syntax.MLC_Int
        (s,FStar_Pervasives_Native.Some uu____6628) ->
        failwith
          "impossible: machine integer not desugared to a function call"
    | FStar_Extraction_ML_Syntax.MLC_Float uu____6643 ->
        failwith "todo: translate_expr [MLC_Float]"
    | FStar_Extraction_ML_Syntax.MLC_Bytes uu____6644 ->
  ...
  fun env  ->
    fun w  ->
      fun op  ->
        fun args  ->
          let uu____6664 =
            let uu____6671 = FStar_List.map (translate_expr env) args  in
            ((EOp (op, w)), uu____6671)  in
          EApp uu____6664
