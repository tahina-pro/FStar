...
4. If you have a working OCaml setup and intend to extract and compile OCaml code
   ...
   required to compile OCaml programs extracted from F\* code:
        ...
        $ opam install ocamlfind batteries stdint zarith yojson ppx_deriving ppx_deriving_yojson ppxlib process
...
4. F\* depends on a bunch of external OCaml packages which you should install using OPAM:
  ...
  ```sh
  $ opam install ocamlbuild ocamlfind batteries stdint zarith yojson fileutils pprint menhir sedlex ppx_deriving ppx_deriving_yojson process ppxlib
  ```
