...
/// Abstract predicate for vprop implication.
...
: Lemma
  ...
  [SMTPatOr [
    [SMTPat (p `can_be_split` r); SMTPat (p `can_be_split` q)];
    [SMTPat (p `can_be_split` r); SMTPat (q `can_be_split` r)];
  ]]
...
/// are equivalent and their selectors are related by a bijection.
...
val vselect (#p: vprop) (x: valid_value p) (q: vprop { p `can_be_split` q }) : GTot (valid_value q)

val vselect_id (#p: vprop) (x: valid_value p) : Lemma
  (vselect x p == x)
  [SMTPat (vselect #p x p)]
...
val vselect_compose (#p1 p2 p3: vprop) (x: valid_value p1) : Lemma
  (requires (
    p1 `can_be_split` p2 /\ p2 `can_be_split` p3
  ))
  (ensures (vselect #p2 (vselect #p1 x p2) p3 == vselect #p1 x p3))
  [SMTPatOr [
    [SMTPat (vselect #p1 x p3); SMTPat (p1 `can_be_split` p2)];
    [SMTPat (vselect #p1 x p3); SMTPat (p2 `can_be_split` p3)];
  ]]

val vselect_pair (#p p1 p2: vprop) (x: valid_value p) : Lemma
  (requires (
    p `can_be_split` (p1 `star` p2)
  ))
  (ensures (
...
unfold
let vselect_norm
  (#p1: vprop) (x: valid_value p1) (p2: vprop { p1 `can_be_split` p2 })
: GTot (normal (t_of p2))
= vselect x p2
...
type ens_t (pre:pre_t) (a:Type) (post:post_t a) =
  valid_value pre -> (x:a) -> valid_value (post x) -> Type0
