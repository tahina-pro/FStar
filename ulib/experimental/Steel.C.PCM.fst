...
unfold
let lem_assoc_l_codom0 (#a :Type u#a) (p: pcm' a) (xyz: lem_assoc_l_dom p) =
  squash (
    let (x, y, z) = xyz in
...
unfold
let lem_assoc_r_codom0 (#a :Type u#a) (p: pcm' a) (xyz: lem_assoc_r_dom p) =
  squash (
    let (x, y, z) = xyz in
    p.composable (y, z) == true /\
    p.composable (x, p.op (y, z)) == true /\
...
: Lemma
  (compatible pcm x x)
= let c : squash (pcm.p.composable (x, pcm.p.one) == true /\ pcm.p.op (x, pcm.p.one) == x) = pcm.is_unit x in
  pcm.comm (one pcm, x);
