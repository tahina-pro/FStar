...
open Steel.V.Effect
friend Steel.V.Effect

#set-options "--warn_error -330"  //turn off the experimental feature warning

let req_to_act_req (#pre:vprop) (req:req_t pre) : mprop (hp_of pre) =
  fun m ->
    interp (hp_of pre) m /\ (value_is_valid_complete pre m; req (sel_of pre m))
...
= fun m0 x m1 -> interp (hp_of pre) m0 /\ interp (hp_of (post x)) m1 /\
  (value_is_valid_complete pre m0; value_is_valid_complete (post x) m1;
...
let frame00 #a #framed #opened #obs #pre #post #req #ens f frame =
  fun frame' ->
      ...
      vselect_correct (pre `star` frame) pre (core_mem m0);
      vselect_correct (pre `star` frame) frame (core_mem m0);
      let snap = Ghost.hide (sel_of frame (core_mem m0)) in
      ...
      vselect_correct (post x `star` frame) frame (core_mem m1);
      vselect_correct (post x `star` frame) (post x) (core_mem m1);
...
let bind a b opened o1 o2 #framed_f #framed_g #pre_f #post_f #req_f #ens_f #pre_g #post_g #req_g #ens_g #frame_f #frame_g #post #_ #_ #p #p2 f g =
  fun frame ->
    ...
    let h0 = Ghost.hide (sel_of (pre_f `star` frame_f) (core_mem m0)) in
    ...
    let h1 = Ghost.hide (sel_of (post_f x `star` frame_f) (core_mem m1)) in

    vselect_correct (post_f x `star` frame_f) (pre_g x `star` frame_g x) (core_mem m1);
    let h1' = Ghost.hide (vselect h1 (pre_g x `star` frame_g x)) in
    ...
    vselect_correct
      ...
      (pre_g x)
      (core_mem m1);
    assert (vselect h1' (pre_g x) == vselect h1 (pre_g x));
    ...
    let h2' = Ghost.hide (sel_of (post_g x y `star` frame_g x) (core_mem m2)) in
    vselect_correct
      (post_g x y `star` frame_g x)
      (post y)
      (core_mem m2);
    let h2 = Ghost.hide (vselect h2' (post y)) in
...
let subcomp a opened o1 o2 #framed_f #framed_g #pre_f #post_f #req_f #ens_f #pre_g #post_g #req_g #ens_g #fr #_ #p1 #p2 f =
  fun frame ->
    ...
    let h0 = Ghost.hide (sel_of pre_g (core_mem m0)) in
    ...
    let h0' = Ghost.hide (vselect h0 (pre_f `star` fr)) in
    vselect_correct (pre_f `star` fr) pre_f (core_mem m0);
    vselect_correct (pre_f `star` fr) fr (core_mem m0);
    let x = frame00 f fr frame in

    let m1:full_mem = NMSTTotal.get () in
    value_is_valid_complete (post_f x `star` fr) (core_mem m1);
    let h1' = Ghost.hide (sel_of (post_f x `star` fr) (core_mem m1)) in
    vselect_correct (post_f x `star` fr) (post_g x) (core_mem m1);
    let h1 = Ghost.hide (vselect h1' (post_g x)) in
    
    // Replace h0'/h1' by h0/h1
    vselect_correct (post_f x `star` fr) (post_f x) (core_mem m1);
    vselect_correct (post_f x `star` fr) fr (core_mem m1);
...
let get0 (#opened:inames) (#p:vprop) (_:unit) : repr (erased (valid_value p))
  true opened Unobservable p (fun _ -> p)
  (requires fun _ -> True)
  (ensures fun h0 r h1 -> h0 == h1 /\ Ghost.reveal r == h0)
  = fun frame ->
      let m0:full_mem = NMSTTotal.get () in
      value_is_valid_complete p (core_mem m0);
      let h0 = Ghost.hide (sel_of p (core_mem m0)) in
      h0
...
let change_slprop0 (#opened:inames) (p q:vprop) (vp:erased (t_of p)) (vq:erased (t_of q))
  ...
  ) : repr unit false opened Unobservable p (fun _ -> q) (fun h -> reveal vp == h) (fun _ _ h1 -> reveal vq == h1)
  = fun frame ->
      let m:full_mem = NMSTTotal.get () in
      proof (core_mem m);
      Classical.forall_intro (Classical.move_requires proof);
...
let change_equal_slprop
  p q
=
  let x : Ghost.erased (t_of p) = gget p in
...
let change_slprop_20 (#opened:inames) (p q:vprop) (vq:erased (t_of q))
  (proof:(m:mem) -> Lemma
    ...
    (ensures interp (hp_of q) m /\ sel_of q m == reveal vq)
           ...
           (fun _ -> True) (fun _ _ h1 -> reveal vq == h1)
  = fun frame ->
      let m:full_mem = NMSTTotal.get () in
      proof (core_mem m);
      Classical.forall_intro (Classical.move_requires proof);
  ...
  (proof:(m:mem) -> Lemma
    ...
    (ensures
      ...
      rel (sel_of p m) (sel_of q m))
           ...
           (fun _ -> True) (fun h0 _ h1 -> rel h0 h1)
  = fun frame ->
      let m:full_mem = NMSTTotal.get () in
      proof (core_mem m);
      let h0 = sel_of p (core_mem m) in
      let h1 = sel_of q (core_mem m) in
  ...
  (proof:(m:mem) -> Lemma
    ...
    (ensures
      ...
      rel (sel_of p m) (sel_of q m))
           ...
           (fun m -> cond m) (fun h0 _ h1 -> rel h0 h1)
  = fun frame ->
      let m:full_mem = NMSTTotal.get () in
...
let extract_info0 (#opened:inames) (p:vprop) (vp:erased (normal (t_of p))) (fact:prop)
  ...
  = fun frame ->
      ...
      let h0 = sel_of p (core_mem m0) in
      l (core_mem m0)
...
: SteelGhost unit opened
  (vdep v p)
  (fun _ -> v `star` q)
  (requires (fun h -> q == p (dfst (vselect_norm h (vdep v p)))))
  (ensures (fun h _ h' ->
      let fs = vselect_norm h' v in
      let sn = vselect_norm h' q in
