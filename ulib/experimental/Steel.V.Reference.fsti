...
module Steel.V.Reference

open FStar.Ghost
open Steel.FractionalPermission

open Steel.Memory
open Steel.V.Effect.Atomic
open Steel.V.Effect
...
let sel (#a:Type) (#p:vprop) (r:ref a)
  (h:valid_value p{FStar.Tactics.with_tactic selector_tactic (can_be_split p (vptr r) /\ True)})
: GTot a
  = vselect h (vptr r)
...
val readp (#a:Type0) (r:ref a) (p: perm) : Steel a
  ...
  (requires fun _ -> True)
  (ensures fun h0 x h1 -> h0 == h1 /\ h1 == x)
...
val share (#a:Type0) (#uses:_) (#p: perm) (r:ref a)
  : SteelGhost unit uses
    ...
    (fun _ -> True)
    (fun h _ h' ->
      vselect_norm h' (vptrp r (half_perm p)) == h
    )
...
val gather_gen (#a:Type0) (#uses:_) (r:ref a) (p0:perm) (p1:perm)
  : SteelGhost perm uses
    ...
    (fun h res h' ->
      res == sum_perm p0 p1 /\
      vselect_norm h (vptrp r p0) == (h' <: a) /\
      vselect_norm h (vptrp r p1) == (h' <: a)
    )
...
val gather (#a: Type0) (#uses: _) (#p: perm) (r: ref a)
  : SteelGhost unit uses
      ...
      (fun _ -> True)
      (fun h _ h' ->
        (h' <: a) == vselect_norm h (vptrp r (half_perm p))
      )
...
let ghost_sel (#a:Type) (#p:vprop) (r:ghost_ref a)
  (h:valid_value p{FStar.Tactics.with_tactic selector_tactic (can_be_split p (ghost_vptr r) /\ True)})
: GTot a
  = vselect h (ghost_vptr r)
...
val ghost_readp (#a:Type0) (#opened:inames) (r:ghost_ref a)
  ...
  : SteelGhost (Ghost.erased a) opened
    ...
    (requires fun _ -> True)
    (ensures fun h0 x h1 -> h0 == h1 /\ Ghost.reveal x == (h1 <: a))
...
val ghost_write (#a:Type0) (#opened:inames) (r:ghost_ref a) (x:Ghost.erased a)
  : SteelGhost unit opened
      (ghost_vptr r) (fun _ -> ghost_vptr r)
      (requires fun _ -> True)
      (ensures fun _ _ h1 -> Ghost.reveal x == h1)
...
val ghost_share (#a:Type0) (#uses:_) (#p: perm) (r:ghost_ref a)
  : SteelGhost unit uses
    ...
    (fun _ -> True)
    (fun h res h' ->
      vselect_norm h' (ghost_vptrp r (half_perm p)) == h
    )
...
val ghost_gather_gen (#a:Type0) (#uses:_) (r:ghost_ref a) (p0:perm) (p1:perm)
  : SteelGhost perm uses
    ...
    (fun h res h' ->
      res == sum_perm p0 p1 /\
      vselect_norm h (ghost_vptrp r p0) == h' /\
      vselect_norm h (ghost_vptrp r p1) == (h' <: a)
    )
