...
module Steel.V.Effect.Atomic

open Steel.Memory
module T = FStar.Tactics
include Steel.V.Effect.Common
...
let return_ens (a:Type) (x:a) (p:a -> vprop) : ens_t (p x) a p =
  fun h0 r h1 -> r == x /\ vselect h0 (p x) == vselect h1 (p x)
...
= fun m0 ->
  ...
  (forall (x:a) (h1:valid_value (post_f x `star` frame_f)).
...
= fun m0 y m2 ->
  req_f (vselect m0 pre_f) /\
  (exists (x_bind_ens:a) (h1_bind_ens:valid_value (post_f x_bind_ens `star` frame_f)).
    pr x_bind_ens /\
    vselect m0 frame_f == vselect h1_bind_ens frame_f /\
    (post_f x_bind_ens `star` frame_f) `can_be_split` (pre_g x_bind_ens `star` frame_g x_bind_ens) /\
    post y `can_be_split` (post_g x_bind_ens y `star` frame_g x_bind_ens) /\
    vselect h1_bind_ens (frame_g x_bind_ens) == vselect m2 (frame_g x_bind_ens) /\
    ens_f (vselect m0 pre_f) x_bind_ens (vselect h1_bind_ens (post_f x_bind_ens)) /\
    ens_g x_bind_ens (vselect h1_bind_ens (pre_g x_bind_ens)) y (vselect m2 (post_g x_bind_ens y)))
...
= T.rewrite_with_tactic (fun _ -> T.norm normal_steps; T.dump "subcomp_pre"; T.trefl ()) (squash (
    (forall (h0_subcomp_pre:valid_value pre_g). (pre_g `can_be_split` (pre_f `star` frame)) ==> req_g h0_subcomp_pre ==> req_f (vselect h0_subcomp_pre pre_f)) /\
  (forall (h0_subcomp_pre:valid_value pre_g) (x_subcomp_pre:a) (h1_subcomp_pre:valid_value (post_g x_subcomp_pre)). (
     (req_g h0_subcomp_pre /\
     pre_g `can_be_split` (pre_f `star` frame) /\
     post_g x_subcomp_pre `can_be_split` (post_f x_subcomp_pre `star` frame) /\
      ens_f (vselect h0_subcomp_pre pre_f) x_subcomp_pre (vselect h1_subcomp_pre (post_f x_subcomp_pre)) /\
      vselect h0_subcomp_pre frame == vselect h1_subcomp_pre frame
      )
        ==> ens_g h0_subcomp_pre x_subcomp_pre h1_subcomp_pre
  ))
  ))
...
: Pure (repr a framed_g opened_invariants o2 pre_g post_g req_g ens_g)
       (requires (o1 = Unobservable || o2 = Observable) /\
         subcomp_pre req_f ens_f req_g ens_g frame)
       (ensures fun _ -> True)
...
val lift_atomic_steel
  ...
  : Steel.V.Effect.repr a framed pre post req ens

sub_effect SteelAtomicBase ~> Steel.V.Effect.SteelBase = lift_atomic_steel
...
val get (#p:vprop) (#opened:inames) (_:unit) : SteelGhostF (erased (valid_value p))
  opened
  p (fun _ -> p)
  (requires fun _ -> True)
  (ensures fun h0 r h1 -> h0 == h1 /\ Ghost.reveal r == h0)
...
val change_slprop
  ...
  (l:(m:mem) -> Lemma
    ...
    (ensures interp (hp_of q) m /\ sel_of q m == reveal vq)
                    ...
                    (fun h -> reveal vp == h) (fun _ _ h1 -> reveal vq == h1)

val change_equal_slprop (#opened:inames) (p q: vprop)
  : SteelGhost unit opened p (fun _ -> q) (fun _ -> p == q) (fun h0 _ h1 -> p == q /\ h1 == h0)

val change_slprop_2 (#opened:inames) (p q:vprop) (vq:erased (t_of q))
  (l:(m:mem) -> Lemma
    (requires interp (hp_of p) m)
    (ensures interp (hp_of q) m /\ sel_of q m == reveal vq)
  ) : SteelGhost unit opened p (fun _ -> q) (fun _ -> True) (fun _ _ h1 -> reveal vq == h1)
...
val change_slprop_rel (#opened:inames) (p q:vprop)
  ...
  ) : SteelGhost unit opened p (fun _ -> q) (fun _ -> True) (fun h0 _ h1 -> rel h0 h1)
...
val change_slprop_rel_with_cond (#opened:inames)
  ...
  (l:(m:mem) -> Lemma
    ...
    (ensures interp (hp_of q) m /\
      rel (sel_of p m) (sel_of q m))
  ) : SteelGhost unit opened p (fun _ -> q) (fun h0 -> cond h0) (fun h0 _ h1 -> rel h0 h1)
...
val reveal_star (#opened:inames) (p1 p2:vprop)
 : SteelGhost unit opened (p1 `star` p2) (fun _ -> p1 `star` p2)
   (requires fun _ -> True)
   (ensures fun h0 _ h1 ->
     vselect h0 p1 == vselect h1 p1 /\
     vselect h0 p2 == vselect h1 p2 /\
     vselect_norm h0 (p1 `star` p2) == (vselect h0 p1, vselect h0 p2) /\
     vselect_norm h1 (p1 `star` p2) == (vselect h1 p1, vselect h1 p2)
   )
...
: SteelGhost unit opened v (fun _ -> vrefine v p)
  (requires fun h -> p (vselect h v))
  (ensures fun h _ h' -> h == (h' <: t_of (vrefine v p)))
...
: SteelGhost unit opened (vrefine v p) (fun _ -> v)
  (requires fun _ -> True)
  (ensures fun h _ h' -> h' == (h <: t_of (vrefine v p)) /\ p h')
...
: SteelGhost unit opened
    (v `star` q)
    (fun _ -> vdep v p)
    (requires (fun h -> q == p (vselect h v)))
    (ensures (fun h _ h' ->
      let x2 = vselect_norm h' (vdep v p) in
      q == p (vselect h v) /\
      dfst x2 == (vselect h v) /\
      dsnd x2 == (vselect_norm h q)
    ))
...
: SteelGhost (Ghost.erased (t_of v)) opened
  ...
  (requires (fun _ -> True))
  (ensures (fun h res h' ->
      let fs = vselect h' v in
      let sn : t_of (p (Ghost.reveal res)) = vselect h' (p (Ghost.reveal res)) in
      let x2 = vselect_norm h (vdep v p) in
      Ghost.reveal res == fs /\
      dfst x2 == fs /\
      dsnd x2 == sn
