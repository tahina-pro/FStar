...
inline let as_requires (#a:Type) (wp:pure_wp a) = wp false (fun x -> True)
inline let as_ensures  (#a:Type) (wp:pure_wp a) (x:a) = ~ (wp true (fun y -> ~(y=x)))
...
val f : x:int -> PURE int (fun is_wlp p -> (is_wlp \/ x > 0) /\ p (x + 1)) 
