module Hacl.Symmetric.Poly1305_64.Bigint
...
(* Big integer types *)
let bigint = b:buffer H64.t{length b = norm_length}
...
val eval: h:mem -> b:bigint{live h b} -> n:nat{n <= length b} -> GTot nat
let rec eval h b n =
  match n with
  | 0 -> 0
...
val eval_def: h:mem -> b:bigint{live h b} -> n:nat{n <= length b} -> Lemma 
  ((n = 0 ==> eval h b n = 0) /\
   (n <> 0 ==> eval h b n =
...
let rec eval_bytes h b n =
  match n with
  | 0 -> 0
  | _ -> pow2 (8 * (n-1)) * H8.v (get h b (n-1)) + eval_bytes h b (n-1)

(*  Returns the maximum value of a bigint between cell 0 and cell (l-1) *)
val maxValue: h:mem -> b:bigint{live h b} -> l:pos{l <= length b} -> GTot nat
...
val maxValue_increases: h:mem -> b:bigint{live h b}
...
val maxValue_lemma_aux: h:mem -> b:bigint{live h b} -> l:pos{l <= length b} -> Lemma
...
val maxValue_lemma: h:mem -> b:bigint{live h b /\ length b > 0} -> Lemma
  (forall (i:nat). {:pattern (v (get h b i))}
...
val maxValue_bound_lemma_aux: h:mem -> b:bigint{live h b /\ length b > 0}
...
val maxValue_bound_lemma: h:mem -> b:bigint{live h b /\ length b > 0} -> bound:nat ->
  Lemma
...
val maxValueNorm: h:mem -> b:bigint{live h b /\ length b >= norm_length} -> GTot nat
let maxValueNorm h b = maxValue h b norm_length

(* Returns the index of the maximum value *)
val maxValueIdx: h:mem -> b:bigint{live h b} -> l:pos{l <= length b} -> GTot nat
...
val maxValue_eq_lemma: ha:mem -> hb:mem -> a:bigint{live ha a} -> 
...
val maxValueNorm_eq_lemma: ha:mem -> hb:mem ->
...
val eval_eq_lemma: ha:mem -> hb:mem -> a:bigint{live ha a} -> b:bigint{live hb b} ->
...
val norm_eq_lemma: ha:mem -> hb:mem -> a:bigint -> b:bigint -> Lemma 
...
val eval_partial_eq_lemma: ha:mem -> hb:mem -> a:bigint{live ha a} -> b:bigint{live hb b} -> ctr:nat -> len:nat{ ctr <= len /\ len <= length a /\ len <= length b} -> Lemma
...
val eval_null: h:mem -> b:bigint{live h b} -> len:nat{len <= length b} -> Lemma
...
val max_value_of_null_lemma: h:mem -> b:bigint{live h b /\ length b > 0} ->
