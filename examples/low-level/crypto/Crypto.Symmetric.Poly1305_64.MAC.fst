module Crypto.Symmetric.Poly1305_64.MAC
...
let itext: Type0 = if Flag.mac_log then PS.text else unit

type log = option (itext * PS.tag) // option (Seq.seq elem * word16)
...
noeq type state (i:id) =
  | State:
      ...
      s: wordB_16 {frameOf #PS.byte s = region} ->
      log: log_ref region ->
      state i

let genPost0 (i:id) (region:rid{is_eternal_region region}) m0 (st: state i) m1 =
    ~(contains m0 st.r) /\
    ~(contains #PS.byte m0 st.s) /\
    st.region == region /\
    norm m1 st.r /\
    Buffer.live #PS.byte m1 st.s /\
    (mac_log ==>
...
let alloc i region key =
  ...
  let zero_8:PS.byte = if mac_log then 0uy else Hacl.Cast.uint8_to_sint8 0uy in
...
let acc_inv'(#i:id) (st:state i) (acc:accBuffer i) h =
  ...
  (mac_log ==> (
    ...
    sel_elem h acc.a == PS.poly log (sel_elem h st.r)))
...
val seq_head_snoc: #a:Type -> xs:Seq.seq a -> x:a ->
  Lemma (requires True)
        (ensures Seq.length (SeqProperties.snoc xs x) > 0 /\
                 PS.seq_head (SeqProperties.snoc xs x) == xs)
let seq_head_snoc #a xs x =
  Seq.lemma_len_append xs (Seq.create 1 x);
  Seq.lemma_eq_intro (PS.seq_head (SeqProperties.snoc xs x)) xs
...
val mac: #i:id -> st:state i -> l:itext -> acc:accB i -> tag:tagB -> ST unit
  (requires (fun h0 ->
    live #PS.byte h0 tag /\ live #PS.byte h0 st.s /\
    disjoint #Hacl.UInt64.t #PS.byte acc st.s /\ disjoint #PS.byte  tag acc /\ disjoint #PS.byte tag st.r /\ disjoint #PS.byte #PS.byte tag st.s /\
    acc_inv st l acc h0 /\
    (mac_log ==> m_sel h0 (ilog st.log) == None)))
  (ensures (fun h0 _ h1 ->
    live #PS.byte h0 st.s /\ live h0 st.r /\ live #PS.byte h1 tag /\
    // modifies h0 h1 "the tag buffer and st.log" /\
    (mac_log ==>
      (let mac = PS.mac_1305 l (sel_elem h0 st.r) (sel_word h0 st.s) in
...
val verify: #i:id -> st:state i -> l:itext -> computed:accB i -> tag:tagB ->
  ...
  (requires (fun h0 -> mac_log ==>
    sel_elem h0 computed == PS.poly (reveal l) (sel_elem h0 st.r)))
  (ensures (fun h0 b h1 ->
    h0 == h1 /\
    (let mac = PS.mac_1305 (reveal l) (sel_elem h0 st.r) (sel_word h0 st.s) in
