...
let poly1305_finish log st mac m len key =
  push_frame();
...
let crypto_onetimeauth output input len k =
  ...
  let st = {r = r; h = h} in
  let init_log = poly1305_init st k in
  let partial_log = poly1305_blocks init_log st input len in
  let final_log = poly1305_finish partial_log st output input len k in
  pop_frame()
