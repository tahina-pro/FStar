...
#else
  return ((x.iv.high == z.iv.high) && (x.iv.low == z.iv.low) && x.ctr >= z.ctr);
#endif
}
...
#else
FStar_UInt128_t Crypto_Symmetric_PRF_iv_0 = {.low = 0, .high = 0};
#endif
...
)
{
  ...
  if (scrut.tag == Prims_option_____Some)
  {
    ...
    return (Prims_option__uint8_t_ ){ .tag = Prims_option__uint8_t__None, { .case_None = {0} } };
  else
  {
    printf("KreMLin abort at %s:%d\n", __FILE__, __LINE__);
    exit(255);
