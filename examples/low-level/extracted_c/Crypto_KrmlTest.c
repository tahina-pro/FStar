...
bool test()
{
  ...
  TestLib_compare_and_print(string_cipher, expected_cipher, cipher, cipherlen);
  ...
  TestLib_compare_and_print(string_decryption,
    Crypto_Plain_bufferRepr(i, FStar_UInt32_v(plainlen), plain),
    Crypto_Plain_bufferRepr(i, FStar_UInt32_v(plainlen), decrypted),
    plainlen);
  bool
  fail_0 =
...
int32_t main()
{
  ...
  bool
  uu____10993 = test_aes_gcm(i, (uint32_t )1, k0, iv0, aadlen0, aad0, plainlen0, plain0, cipher0);
  ...
  bool uu____10996 = test_aes_gcm(i2, (uint32_t )4, k, iv, aadlen, aad, plainlen, plain, cipher);
  return exit_success;
}
