module VSelectors.Tree.Core

open Steel.Memory
open Steel.V.Effect
open Steel.V.Reference
...
let v_linked_tree
  ...
  (#p:vprop)
  (r:t a)
  (h:valid_value p { FStar.Tactics.with_tactic selector_tactic (can_be_split p (linked_tree r) /\ True) })
    : GTot (tree a)
  = vselect h (linked_tree r)
