...
: SteelSel bool
    ...
    (fun h -> fact_test_pre n dist (h (vptr i), h (vptr res)))// (h (fact_test_vprop i res)))
    (fun _ b h' -> fact_test_post n b dist (h' (vptr i), h' (vptr res)))// (fact_test_vprop i res dist)))
=
  // change_equal_slprop
  //   (fact_test_vprop i res dist)
  //   (vptr i `star` vptr res);
...
: SteelSel (Ghost.erased nat)
    ...
    (fun h -> fact_test_post n true dist (h (vptr i), h (vptr res))) // (h (fact_test_vprop i res dist)))
    (fun _ dist' h' -> fact_test_pre n dist' (h' (vptr i), h' (vptr res)) /\
      //(h' (fact_test_vprop i res dist')) /\
      Ghost.reveal dist' < Ghost.reveal dist)
= let _ = get () in
  // change_equal_slprop
  //   (fact_test_vprop i res dist)
  //   (vptr i `star` vptr res);
