...
= assert (Some? (parse (parse_bounded_integer 3) b));
  ...
  let b1 : bytes32 = Seq.slice b 0 3 in
  ...
  assert (v == x);
  let b1l : bytes32 = Seq.slice b1 0 2 in
  assert (b1l == Seq.slice b 0 2);
  let b1r : bytes32 = Seq.slice b1 2 3 in
  ...
  let b_ : bytes32 = Seq.slice b 2 (Seq.length b) in
