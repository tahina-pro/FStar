...
= let s1 = advance_slice b off1 in
  let s2 = advance_slice s1 off2 in
...
val bslice_prefix_equals (b b': bslice) (h0 h1: HS.mem) :
...
val modifies_slice : b:bslice -> h: HS.mem -> h': HS.mem -> GTot Type0
...
let is_concat_append (#a:Type) (b b1 b2:B.buffer a) h :
    Lemma (requires (is_concat_of b b1 b2 /\
          ...
          (ensures (B.live h b /\
                    B.live h b1 /\
                    B.live h b2 /\
                    B.as_seq h b == B.as_seq h b1 `Seq.append` B.as_seq h b2)) =
    Seq.lemma_eq_intro (B.as_seq h b) (B.as_seq h b1 `Seq.append` B.as_seq h b2)
...
let is_concat_suffix (#a:Type) (b b1 b2:B.buffer a) h :
    Lemma (requires (is_concat_of b b1 b2 /\
          ...
          (ensures (B.live h b /\
                    ...
                    B.length b1 + B.length b2 == B.length b /\
                    B.as_seq h b2 == Seq.slice (B.as_seq h b) (B.length b1) (B.length b))) =
    Seq.lemma_eq_intro (B.as_seq h b2) (Seq.slice (B.as_seq h b) (B.length b1) (B.length b))
...
let is_concat_prefix (#a:Type) (b b1 b2:B.buffer a) h :
    Lemma (requires (is_concat_of b b1 b2 /\
          ...
          (ensures (B.live h b /\
                    ...
                    // just to satisfy the bounds in the refinements to slice
                    B.length b1 + B.length b2 == B.length b /\
                    B.as_seq h b1 == Seq.slice (B.as_seq h b) 0 (B.length b1))) =
    Seq.lemma_eq_intro (B.as_seq h b1) (Seq.slice (B.as_seq h b) 0 (B.length b1))
...
val modifies_prefix_seq_intro (b:bslice) (len: U32.t{U32.v len <= U32.v b.len}) (h0 h1: HS.mem) :
  Lemma (requires (modifies_slice b h0 h1 /\
                   ...
                   begin
                    ...
                    forall (i:nat{len <= i /\ i < U32.v b.len}).{:pattern (Seq.index s0 i); (Seq.index s1 i)}
...
val modifies_prefix_split (b b1 b2:bslice) (len: U32.t{U32.v len <= U32.v b.len}) (h0 h1: HS.mem) :
    Lemma (requires (live h0 b /\
                     live h1 b /\
                     is_concat_of b.p b1.p b2.p /\
...
let modifies_grow_from_b1 (b b1 b2:bslice) (h0 h1: HS.mem) : Lemma
...
let modifies_grow_from_b2 (b b1 b2:bslice) (h0 h1: HS.mem) : Lemma
    (requires (live h0 b /\ live h1 b /\
              is_concat_of b.p b1.p b2.p /\
